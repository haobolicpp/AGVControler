...
PixSurfById DrawPixelTexture(const std::vector<char>& intensity,
                                  const std::vector<char>& alpha,
                                  const int width, const int height,std::vector<uint32_t>* const cairo_data
                                  ) {
  //cairo_data->clear();
  //CHECK(cairo_data->empty());
  ...
  // Properly dealing with a non-common stride would make this code much more
  // complicated. Let's check that it is not needed.
  const int expected_stride = 4 * 1;
  PixSurfById pixelsurfaces;
  CHECK_EQ(expected_stride, cairo_format_stride_for_width(kCairoFormat, 1));
  //unsigned char pixel_value[4];     
  for (size_t i = 0; i < intensity.size(); ++i) {
    
    // We use the red channel to track intensity infor mation. The green
    // channel we use to track if a cell was ever observed.
    const uint8_t intensity_value = intensity.at(i);
    const uint8_t alpha_value = alpha.at(i);
    ...
    //const uint8_t var = 0;
    if(observed != 0){
      PixId pixid{i % width, i / width};
      //pixel_value[0] = reinterpret_cast<unsigned char> (var);
      //pixel_value[1] = reinterpret_cast<unsigned char> (observed);
      //pixel_value[2] = reinterpret_cast<unsigned char> (intensity_value);
      //pixel_value[3] = reinterpret_cast<unsigned char> (alpha_value);
      auto surface = MakeUniqueCairoSurfacePtr(cairo_image_surface_create_for_data(
        reinterpret_cast<unsigned char*>(&(cairo_data->at(i))), kCairoFormat, 1,
        1, 4));
      pixelsurfaces.push_back({pixid, std::move(surface)});
     }
...
PaintSubmapSlicesResult PaintSubmapSlicesByPixel(
    const std::map<::cartographer::mapping::SubmapId, SubmapSlice>& submaps,
    const double resolution) {
  Eigen::AlignedBox2f bounding_box;
  {
  ...
  {
    auto cr = MakeUniqueCairoPtr(cairo_create(surface.get()));
    //cairo_set_source_rgba(cr.get(), 255, 255, 255, 0.);
    cairo_set_source_rgba(cr.get(), 0.5, 0.0, 0.0, 1.);
    cairo_paint(cr.get());
    cairo_translate(cr.get(), origin.x(), origin.y());
    CairoPaintSubmapSlices(1. / resolution, submaps, cr.get(),
                           [&cr](const SubmapSlice& submap_slice) {
                             for(const std::pair<PixId, ::cartographer::io::UniqueCairoSurfacePtr> &pixel_surface:submap_slice.pixel_surfaces){
                                cairo_set_source_surface(
                                    cr.get(), pixel_surface.second.get(), pixel_surface.first.first,pixel_surface.first.second);
                                cairo_paint(cr.get());
                             }
                           });
    cairo_surface_flush(surface.get());
