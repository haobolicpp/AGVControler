...
bool AMCLOdom::UpdateAction(pf_t *pf, AMCLSensorData *data)
{
  ...
  case ODOM_MODEL_DIFF:
  {
    ...
    pf_sample_t* sampleA = set->samples;
    // printf("ERROR: A Before odom pose [%f]\n", sampleA->pose.v[0]);
    // printf("ERROR: A delta_rot1 [%f] trans[%f] rot2[%f]\n", delta_rot1, delta_trans, delta_rot2);
    // printf("ERROR: A alpha3[%f] alpha4[%f]\n", this->alpha3, this->alpha4);
    ...
    // pf_sample_t* sampleB = set->samples;
    // printf("ERROR: B rot1_hat [%f] trans_hat[%f] rot2_hat[%f]\n", delta_rot1_hat, delta_trans_hat, delta_rot2_hat);
    // printf("ERROR: B After odom pose [%f]\n", sampleB->pose.v[0]);
