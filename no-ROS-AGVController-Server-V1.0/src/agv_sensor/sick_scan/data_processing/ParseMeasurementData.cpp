...
#include <data_processing/ParseMeasurementData.h>
  ...
  std::vector<uint8_t>::const_iterator data_ptr, datastructure::MeasurementData& measurement_data)
{
  ...
  {
    printf("Field Number Beams has a value larger then the expected Number of Beams for the "
             "laserscanners. Skipping this measurement.\n");
    printf("Max expected beams: %i\n", maxexpectedbeams);
    printf("Number beams according to the datafield: %i\n", numBeams);
