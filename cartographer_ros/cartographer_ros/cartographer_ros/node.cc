...
void Node::PublishLocalTrajectoryData(const ::ros::TimerEvent& timer_event) {
  ...
  for (const auto& entry : map_builder_bridge_.GetLocalTrajectoryData()) {
    ...
    //LOG(INFO)<<"local_to_map"<<trajectory_data.local_to_map;
    //LOG(INFO)<<"tracking_to_local"<<tracking_to_local;
    if (trajectory_data.published_to_tracking != nullptr) {
      if (node_options_.publish_to_tf) {
        if (trajectory_data.trajectory_options.provide_odom_frame) {
          ...
          //LOG(INFO)<<"trajectory_data.published_to_tracking"<<(*trajectory_data.published_to_tracking);
...
void Node::save(const std::string& pbstream_filename, const std::string& map_filestem,
