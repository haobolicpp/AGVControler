...
LatchedStopRotateController::LatchedStopRotateController(const std::string& name) {
  ...
  // ROS_INFO("---------------latch_xy_goal_tolerance_: %d,name:%s" ,latch_xy_goal_tolerance_, name.c_str());

  latch_xy_goal_tolerance_ = true;//手动修改，上面未成功加载该参数！！
...
bool LatchedStopRotateController::computeVelocityCommandsStopRotate(geometry_msgs::Twist& cmd_vel,
  ...
  if (fabs(angle) <= limits.yaw_goal_tolerance) {
    ...
    //ROS_INFO("Angle: %f(%f) Tolerance: %f, goal_th: %f", angle, angle*180/3.1415 , limits.yaw_goal_tolerance, goal_th);
    geometry_msgs::PoseStamped robot_vel;
    odom_helper_.getRobotVel(robot_vel);
    nav_msgs::Odometry base_odom;
    odom_helper_.getOdom(base_odom);

    //ROS_INFO("odom_z:%f, odom_x:%f,limits.theta_stopped_vel:%f,limits.trans_stopped_vel:%f", base_odom.twist.twist.angular.z,base_odom.twist.twist.linear.x,
    //limits.theta_stopped_vel, limits.trans_stopped_vel);
